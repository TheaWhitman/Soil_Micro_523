...
select(metadata, SampleID, TransectName, Vegetation, pH)
```

To choose rows, use `filter()`:

```{r, purl = FALSE}
filter(metadata, Vegetation == "yes")
```
...
metadata %>%
  ...
  select(SampleID, TransectName, pH)
```
...
> ### Challenge {.challenge}
>
> Using pipes, subset the data to include rows where the Vegetation is 'no'. Retain columns
> `SampleID`, `Transect`, and `Total_Organic_C`
...
metadata %>%
  mutate(TOC_fract = Total_Organic_C / 100)
```
...
vegetation status and find the number of rows of data for each 
status, we would do:
...
So to view mean `Total_Organic_C` by vegetation status:

```{r, purl = FALSE}
metadata %>%
  group_by(Vegetation) %>%
  summarize(mean_TOC = mean(Total_Organic_C, na.rm = TRUE))
```
...
```{r, purl = FALSE}
metadata %>%
  group_by(Vegetation, TransectName) %>%
  summarize(mean_TOC = mean(Total_Organic_C, na.rm = TRUE))
