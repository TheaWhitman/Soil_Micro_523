...
Each point represents a sample and the value on the x-axis is the sample index in the file, where the values on the y-axis correspond to the total organic carbon for that sample. For any plot you can customize many features of your graphs (fonts, colors, axes, titles) through [graphic options](http://www.statmethods.net/advgraphs/parameters.html)
...
```{r, fig.align='center'}
plot(TOC, pch=8, main="Scatter plot of soil total organic C")
```
...
Using additional information from our metadata, we can use plots to compare values between the different vegetation status using a **boxplot**. A boxplot provides a graphical view of the median, quartiles, maximum, and minimum of a data set. 

```{r boxplot, fig.align='center'}
# Boxplot
boxplot(TOC ~ Vegetation, metadata)
```

Similar to the scatterplots above, we can pass in arguments to add in extras like plot title, axis labels and colors.

```{r, fig.align='center'}
boxplot(TOC ~ Vegetation, metadata,  col=c("pink","darkgrey"),
        main="Vegetation presence/absence", ylab="Total Organic Carbon")
```
...
```{r, fig.align='center'}
ggplot(metadata) +
  geom_point(aes(x = SampleID, y= Total_Organic_C))
```
...
```{r, fig.align='center'}
ggplot(metadata) +
  geom_histogram(aes(x = Total_Organic_C))
...
```{r, fig.align='center'}
ggplot(metadata) +
  geom_histogram(aes(x = Total_Organic_C), stat = "bin", binwidth=100)
...
```{r, fig.align='center'}
ggplot(metadata) +
  geom_boxplot(aes(x = Vegetation, y = Total_Organic_C, fill = Vegetation)) +
  ggtitle('Boxplot of TOC by vegetation status') +
  xlab('Vegetation status') +
  ylab('Total Organic C') +
...
ggplot(example_data) +
  geom_boxplot(aes(x = Vegetation, y =....) +
  ggtitle(...) +
  xlab(...) +
  ylab(...) +
