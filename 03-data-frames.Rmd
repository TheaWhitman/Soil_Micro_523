...
```{r, eval=FALSE}
# Select the Vegetation column from metadata
metadata[ , "Vegetation"]

# Alternatively, as we saw earlier...
metadata$Vegetation
```

The first method allows you to select multiple columns at once. Suppose we wanted Vegetation and pH information:

```{r, eval=FALSE}
metadata[, c("Vegetation", "pH")]
```

You can even access columns by column name _and_ select specific rows of interest. For example, if we wanted the Vegetation and pH for just rows 4 through 7, we could do:

```{r, eval=FALSE}
metadata[4:7, c("Vegetation", "pH")]
```
